...
public class ExceptionHandling {
	...
	public static void main(String[] args) {
		...
		try { // 실행하고싶은 내용을 블럭으로 감싼다.
			 int result = 10 / 0;
			 System.out.println(result); // try 없이 실행하면 콘솔창에 예외 발생
		} catch(ArithmeticException | IndexOutOfBoundsException e) { //try-catch로 감싸주기만해도 예외처리가능
			 e.printStackTrace();  // 예외메세지가 똑같이 출력되는 메서드
		} catch(NullPointerException e) {
			 
		} catch(Exception e) { // 모든예외처리를 전부 처리할 수 있음
