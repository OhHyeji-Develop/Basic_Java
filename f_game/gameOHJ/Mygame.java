...
public class Mygame {
	...
	Mygame() {
		h = new Hotel("대덕", "안녕이"); // 먼저 호텔, 관리자 이름 입력
		g = new Guest();
	}
	...
	void first() {
		...
		while (true) { // while문을 실행
			...
			case 1:
				g.reserve(h);
				break; // Guest클래스에 있는 reserve메서드 실행
			case 2:
				start();
				break; // start메서드 실행
			case 3:
	...
	void start() {
		...
		do {
			...
			System.out.println("1.호텔정보 2.일하기");
			System.out.println("3.홈페이지가기 0.마감");
			System.out.println("- - - - - - - - - - - -");
			input = ScanUtil.nextInt();
